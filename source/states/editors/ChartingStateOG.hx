...
class ChartingStateOG extends MusicBeatChartingState
{
	...
	final mouse_listeners:Array<openfl.events.EventType<openfl.events.MouseEvent>> = [MouseEvent.MOUSE_DOWN, MouseEvent.RIGHT_MOUSE_DOWN, MouseEvent.MOUSE_MOVE];
	var eventStuff:Array<Array<String>> = [
		// Events are handled by Events class.
	];
	...
	**/
	...
	override function create()
	{
		...
		if (_song.mania == 0)
			s_termination = "";
		DiscordClient.changePresence("Mixtape Chart Editor",
			StringTools.replace(_song.song, '-', ' ')
			+ " ("
			+ (_song.mania + 1)
			+ " key"
			+ s_termination
	...
	function addSongUI():Void
	{
		...
		var player4DropDown = new FlxUIDropDownMenu(player2DropDown.x + 140, player2DropDown.y, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true),
			function(character:String)
			{
		...
		var player5DropDown = new FlxUIDropDownMenu(player4DropDown.x + 20, player4DropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true),
			function(character:String)
			{
		...
		if (Difficulty.list.length <= 0 || Difficulty.list == [])
			Difficulty.list = Difficulty.defaultList;
		var difficultyDropDown = new FlxUIDropDownMenu(stageDropDown.x, player1DropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(Difficulty.list, true),
	...
	function loadSongNoteTypes() {
		...
		var songName = Paths.formatToSongPath(_song.song);
				...
				noteTypeList = backend.Paths.crawlMulti([
					'assets/data/$songName',
					'assets/shared/data/$songName'], 'txt', null, true);
		#end

		for (file in noteTypeList) {
			if (file.toLowerCase().endsWith("noteTypes.txt".toLowerCase())) {
	...
	function addEventsUI():Void
	{
		...
		var directories:Array<String> = [];
		// states.editors.ChartEditorEvents.
	...
	function addChartingUI()
	{
		...
		waveformTrackDropDown = new FlxUIDropDownMenu(10, 90, FlxUIDropDownMenu.makeStrIdLabelArray(trackNamesArray, false), selectTrack);
		waveformTrackDropDown.selectedId = "None"; // curSelectedTrackName;
		blockPressWhileScrolling.push(waveformTrackDropDown);

		trackVolumeSlider = new FlxUISlider(this, '_curTrackVolume', waveformTrackDropDown.x + 150 - 10, waveformTrackDropDown.y - 15, 0.0, 1.0,
			Math.floor(waveformTrackDropDown.width), null, 5, FlxColor.WHITE, FlxColor.BLACK);
	...
	function loadSong():Void
	{
		...
		if (_song.extraTracks != null && _song.extraTracks.length > 0)
		{
			for (trackName in _song.extraTracks)
			{
				...
				if (Std.isOfType(file, Sound) || OpenFlAssets.exists(file))
				{
					...
					soundTracksMap.set(trackName, newTrack.loadEmbedded(file));
		...
		if (Std.isOfType(file, Sound) || OpenFlAssets.exists(file))
		{
			soundTracksMap.set("Opponent Vocals", opponentVocals.loadEmbedded(file));
		...
		if (Std.isOfType(file, Sound) || OpenFlAssets.exists(file))
		{
			soundTracksMap.set("GF Vocals", gfVocals.loadEmbedded(file));
		...
		try
		{
			if (_song.needsVoices)
			{
				...
				var oppVocals = Paths.voices(_song.song,
					(characterData.vocalsP2 == null || characterData.vocalsP2.length < 1) ? 'opponent' : characterData.vocalsP2);
				if (oppVocals != null)
				{
	...
	override function closeSubState()
	{
		if (playtesting)
		{
			...
			if (gfVocals != null)
			{
				...
				if (voicesgfVolume != null)
					gfVocals.volume = voicesgfVolume.value;
				if (check_mute_vocals_gf != null && check_mute_vocals_gf.checked)
					gfVocals.volume = 0;
			}
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.overlaps(curRenderedNotes) && !FlxG.mouse.justPressed)
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.pressed.V && FlxG.keys.pressed.CONTROL)
				pasteNotes(true, true);
		...
		bpmTxt.text = "Song Name: "
			...
			+ " / "
			+ FlxStringUtil.formatTime(FlxMath.roundDecimal(FlxG.sound.music.length / 1000, 2), false)
			+ "\n"
	...
	function updateZoom()
	{
		...
		zoomTxt.text = 'Zoom: ' + zoomThing + "\n\n\nNote Balance:" + "\n\nNotes: " + notes1 + " / " + notes2 + " (" + (notes2 - notes1) + ")" + "\nLifts: "
			+ holds1 + " / " + holds2 + " (" + (holds2 - holds1) + ")" + "\nSustain: " + Math.round(score1sus) + " / " + Math.round(score2sus) + " ("
			+ Math.round(score2sus - score1sus) + ")" + "\nHurts: " + hurts1 + " / " + hurts2 + " (" + (hurts2 - hurts1) + ")"
	...
	function reloadGridLayer()
	{
		...
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Note.ammo[_song.mania] * 2,
			Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Note.ammo[_song.mania] * 2,
				leHeight + Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
		...
		if (foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0,
				gridBG.height).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * Note.ammo[_song.mania] * 2), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
	...
	function updateHeads():Void {
		// Always set P1 and P4 on the left
		leftIcon.changeIcon(characterData.iconP1);
		if (_song.notes[curSec].exSection) {
			leftIcon.changeIcon(characterData.iconP4);
		}
	
		// Always set P2 and P5 on the right
		rightIcon.changeIcon(characterData.iconP2);
		if (_song.notes[curSec].exSection) {
			rightIcon.changeIcon(characterData.iconP5);
		}
	
		// Move P3 based on GFSection
		if (_song.notes[curSec].gfSection && _song.notes[curSec].mustHitSection) {
	...
	// function mustHitNote(note:Dynamic):Bool
	// {
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if (i[1] > Note.ammo[_song.mania] - 1)
		...
		if (strumLineNotes.members[d].overlaps(curRenderedNotes))
		{
			curRenderedNotes.forEachAlive(function(note:Note)
			{
				if (note.overlapsPoint(new FlxPoint(strumLineNotes.members[d].x + 1, strumLine.y + 1))
					&& note.noteData == d % Note.ammo[_song.mania])
				{
					// trace('tryin to delete note...');
					if (!delnote)
						deleteNote(note);
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if (noteData > -1)
		{
			_song.notes[curSec].sectionNotes.push([
				noteStrum,
				noteData % (Note.ammo[_song.mania] * 2),
				noteSus,
				curNoteTypes[daType]
			]);
		...
		if (FlxG.keys.pressed.CONTROL && noteData > -1)
		{
			_song.notes[curSec].sectionNotes.push([
				noteStrum,
				(noteData + Note.ammo[_song.mania]) % (Note.ammo[_song.mania] * 2),
				noteSus,
				curNoteTypes[daType]
			]);
		}
